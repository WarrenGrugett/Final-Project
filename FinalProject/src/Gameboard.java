   ...
   public void actionPerformed(ActionEvent e)
   {
            ...
            else
            {
               ...
               if (target != null && tower instanceof Tank)
               {
                  for (int j = 0; j < troops.size(); j++)
                  {
                     ...
                     if (Math
                           .abs(troop.x() + GRID_WIDTH / 2 - target.x() + GRID_WIDTH / 2) <= ((Tank) tower)
      ...
      if (sentTroop != null)
      {
         ...
         if (delay == 80)
         {
            ...
            if (sentTroop.x != -1)
            {
               troops.add(((Troop) V.TROOPS.get(sentTroop.x)).clone(map.startPoint().y, map.startPoint().x, true, level));
      ...
      if (mouseX > this.height)
      {
         if (mouseY % height < 0.9f * height)
         {
            ...
            else if (y < V.NUM_UNITS)
            {
               ...
               {
                  Troop troop = ((Troop) V.P_UNITS.get(y)).clone(map.endPoint().y, map.endPoint().x, false, level);
                  troop.orientate(map);
                  troops.add(troop);
