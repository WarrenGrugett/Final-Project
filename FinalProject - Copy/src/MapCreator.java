...
public class MapCreator extends PApplet {
	...
	private int loc, selected = -1, mWidth = 15;
	private float tWidth = 64;
	...
	private void drawMapCreator() {
		...
		for (int y = 0; y < mWidth; y++) {
			for (int x = 0; x < mWidth; x++) {
				if (mapData[y][x] != -1) {
					if (mapData[y][x] != 0)
						text(mapData[y][x], (x + 0.5f) * tWidth, (y + 0.5f) * tWidth);
					stroke(100);
					rect(x * tWidth, y * tWidth, tWidth, tWidth);
					stroke(0);
				}
			}
		}
	...
	private void drawMenu() {
		...
		int num = (troopSelect) ? Data.NUM_TROOPS + 5 : menuItems.length + 3;
		...
		fill(200);
		rect(width - shopWidth, this.height - 3 * height, shopWidth, 0.9f * height);
		...
		fill(0);
		text((troopSelect) ? "Edit Map" : "Select Troops", width - shopWidth / 2, this.height - 2.5f * height);
	...
	public void mouseDragged() {
		if (mouseX < height) {
			int y = (int) (mouseY / tWidth), x = (int) (mouseX / tWidth);
			if (selected != -1)
		...
		if (mouseX > this.height) {
			if (mouseY % height < 0.9f * height) {
				...
				if (troopSelect) {
					...
					try {
						int width = Integer
								.parseInt(JOptionPane.showInputDialog(frame, "How many tiles wide should the map be?"));
						mWidth = width;
					} catch (NumberFormatException e) {
						JOptionPane.showMessageDialog(frame, "Invalid dimensions entered");
					}
				} else if (y == num - 3) {
					troopSelect = true;
					selected = -1;
